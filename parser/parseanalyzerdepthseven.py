...
# Configure logging
logging.basicConfig(filename='parser2errorssciencd.log', level=logging.ERROR, format='%(asctime)s - %(levelname)s - %(message)s')
...
try:
    ...
    if response.status_code == 200:
        ...
        div_elements = soup.find_all('div', class_='tab-pane')

        # Get today's date in the specified format
        today_date = date.today().strftime('%Y-%m-%d')

        # Write the headlines and URLs to a file with the specified format
        with open('sciencedaily_autism_news.txt', 'w', encoding='utf-8') as report_file:
            ...
            for div_element in div_elements:
                ...
                if a_tag:
                    ...
                    url = "https://www.sciencedaily.com" + a_tag['href']
                    report_file.write(f"{headline}\t{url}\n")

            report_file.write('-' * 80 + '\n')

        print("Data written to sciencedaily_autism_news.txt")
    else:
